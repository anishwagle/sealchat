...
const COMPONENT = 'api/protected/friend/getSentRequest';
const FUNCTION = 'GET';

export async function GET(request: Request) {
  Logger.log(COMPONENT, FUNCTION, 'info', 'Fetching sent requests');
  ...
  try {
    ...
    const profiles = await friendService.getSentRequestList(currentUserId);
    
    // Enrich profiles with social status
    const enrichedRequests = await Promise.all(
      profiles.map(async (profile) => ({
        ...profile,
        friendshipStatus: 'sent' as const,
    ...
    Logger.log(COMPONENT, FUNCTION, 'info', 'Sent requests fetched', { count: enrichedRequests.length });
