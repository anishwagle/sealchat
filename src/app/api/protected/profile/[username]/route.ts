...
const COMPONENT = 'ProfileApi';
const FUNCTION = 'GET';

export async function GET(request: Request, { params }: { params: { username: string } }) {
  Logger.log(COMPONENT, FUNCTION, 'info', 'Fetching profile', { username: params.username });

  try {
    
    const user = await userService.findUserByUsername(params.username);
    if (!user) {
      Logger.log(COMPONENT, FUNCTION, 'error', 'User not found', { username: params.username });
      return NextResponse.json({ message: 'User not found', code: 'USER_NOT_FOUND' }, { status: 404 });
    }

    Logger.log(COMPONENT, FUNCTION, 'info', 'Profile fetched', { username: params.username });
