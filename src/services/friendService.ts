...
export class FriendService implements IFriendService {
  async getFriendCount(userId: string): Promise<number> {
    ...
    Logger.log(COMPONENT,FUNCTION,'debug',"get User's Friend Count",{userId});
    const [queryResult] = await pool.query('SELECT * FROM friends WHERE user_id_1=? OR user_id_2=?',[userId,userId]);
    const count = (queryResult as any[]).length;
    Logger.log(COMPONENT,FUNCTION,'debug','Calculated Profile friend Count', {count:count});
    return count;
  }
  async toggleProfileLike(userId1: string, userId2: string): Promise<void> {
    ...
    if(result){
      ...
      await pool.query('DELETE FROM follows WHERE follower_id=? AND followed_id=?',[
        userId1,
        userId2
      ]);
    }else{
      Logger.log(COMPONENT,FUNCTION,"debug","Profile Liked");
      await pool.query('INSERT INTO follows (follower_id,followed_id) VALUES(?,?)',[
        userId1,userId2
      ]);
    }
  ...
  async getProfileLikeCount(userId: string): Promise<number> {
    ...
    Logger.log(COMPONENT,FUNCTION,'debug',"get User's Like Count",{userId});
    const [queryResult] = await pool.query('SELECT * FROM follows WHERE followed_id=?',[userId]);
    const count = (queryResult as any[]).length;
    Logger.log(COMPONENT,FUNCTION,'debug','Calculated Profile Like Count', {count:count});
    return count;
  }
...
async getProfileLikeStatus(userId1: string, userId2: string): Promise<boolean> {
    const FUNCTION = "getProfileLikeStatus";
    Logger.log(COMPONENT, FUNCTION, "debug", "get profile like status", {
      userId1,
      userId2,
    });
    const [queryResult] = await pool.query("SELECT * FROM follows WHERE follower_id=? AND followed_id=?",
        [userId1,userId2]
    );
    const result = (queryResult as any[])[0];
    Logger.log(COMPONENT,FUNCTION,"debug","Get Profile Like Status",{result});
    return !!result;
  }
