...
export class MockUserService implements IUserService {
    ...
    Logger.log(COMPONENT, FUNCTION, 'debug', 'Storing refresh token', { userId });
    refreshTokens[userId] = refreshToken;
  }

  async validateRefreshToken(userId: string, refreshToken: string): Promise<boolean> {
    const FUNCTION = 'validateRefreshToken';
    Logger.log(COMPONENT, FUNCTION, 'debug', 'Validating refresh token', { userId });
    return refreshTokens[userId] === refreshToken;
  }

  async removeRefreshToken(userId: string): Promise<void> {
    const FUNCTION = 'removeRefreshToken';
    Logger.log(COMPONENT, FUNCTION, 'debug', 'Removing refresh token', { userId });
